...
{
    // Declare variables:
    int menu_option, map_option = 0, validation_counter = 0, counter_adventures = 0;
    ...
    MapT maps[] = {
        ...
        createMap("Mapa 6", "Mapa 6", FILE_PATH_PREFIX "Map6.txt"),
        createMap("Mapa 7", "Mapa  7", FILE_PATH_PREFIX "Map7.txt"),
        createMap("Mapa 8", "Mapa  8", FILE_PATH_PREFIX "Map8.txt")};
    ...
    do
    {
        ...
        case 1:
            ...
            if (map_option >= 1 && map_option <= map_menu.num_options)
            {

                // 2. Get map (0 = center, 1 = exterior).
                map_option = getMap(&maps[map_option - 1], adventurers, map_option, 1, &counter_adventures);
            }
            else
            {
                map_option = 0;
        ...
        case 3:
            if (map_option >= 1 && map_option <= map_menu.num_options)
            {
                ...
                {
                    for (int i = 0; i < counter_adventures; i++)
                    {
                        drawMap(maps[map_option - 1], adventurers, MAGENTA_COLOR, CYAN_COLOR, WHITE_COLOR, RED_COLOR);
                        ...
                        if (i == counter_adventures - 1)
                        {
                            if (validation_counter == counter_adventures)
                            {
                                goal_validation = 0;
                            }
