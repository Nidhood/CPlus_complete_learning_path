...
char adventurersMovement(AdventurerT *adventurer, MapT *map)
{
    ...
    case 2:
        return FSM_RIGHT_HAND_LAY(adventurer, map);

    case 3:
        return FSM_RANDOM_DIRECTION(adventurer, map);
...
char FSM_PROBABILITIES(AdventurerT *adventurer, MapT *map)
{
    ...
    case STATE_MOVE_BACKWARDS:
        ...
        if (adventurer->steps_counter > 0)
        {
            ...
            // Returns to the check probabilites::
            adventurer->currentState = STATE_CHECK_PROBABILITIES;
            ;
        }
        else
...
char FSM_RIGHT_HAND_LAY(AdventurerT *adventurer, MapT *map)
{
    ...
    case STATE_MOVE_BACKWARDS:
        ...
        {
            // There is no more steps to go backwards, so we change direction:
            adventurer->currentState = STATE_REAPPEAR;
        }
        adventurer->total_states++;
        return 1;
    ...
    case STATE_TURN_LEFT:
        ...
        else if (strcmp(adventurer->direction.description, "Izquierda") == 0) // Corregir el nombre "Izquierda"
        {
            setAdventurerPosition("Abajo", adventurer);
        }
...
char FSM_RANDOM_DIRECTION(AdventurerT *adventurer, MapT *map)
{
    ...
    // Generate a random number (0 or 1) to decide between moving right or left
    ...
    case STATE_MOVE_FORWARD:
        ...
        else
        {
            ...
            // Transition to the random choice:
            adventurer->currentState = STATE_RANDOM_CHOICE;
        }
        adventurer->total_states++;
        return 1;

    case STATE_RANDOM_CHOICE:
